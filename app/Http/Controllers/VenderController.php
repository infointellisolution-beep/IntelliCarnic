...
class VenderController extends Controller
{
    ...
    public function cobrar(Request $request)
    {
        ...
        \Illuminate\Support\Facades\DB::transaction(function () use ($data, $tipoVenta, $cliente, &$venta, &$articulosActualizados) {
            $cajaActiva = \App\Models\CajaSesion::query()->where('estado', 'abierta')->lockForUpdate()->first();
            ...
            foreach ($data['items'] as $item) {
                ...
                $articulo = Articulo::lockForUpdate()->find($item['articulo_id']);
                if ($articulo) {
                    ...
                    if ($modoInventario === 'dinamico') {
                        ...
                        if ($codigoScanned) {
                            ...
                            if (!$specificLot) {
                                if (preg_match('/10([a-zA-Z0-9]+?)(?=11|15|17|21|310|320|$)/', $cleanCode, $mLote)) {
                                    $loteVal = $mLote[1];
                                    $specificLot = \App\Models\CompraDetalle::where('articulo_id', $articulo->id)
                                        ->where('lote', $loteVal)
                                        ->where('cantidad_peso', '>', 0)
                        ...
                        if ($specificLot) {
                            $rem = round((float) $specificLot->cantidad_peso - $pesoAVender, 3);
                            $specificLot->cantidad_peso = max(0, $rem);
                            $specificLot->save();
                        } else {
                            $lotes = \App\Models\CompraDetalle::where('articulo_id', $articulo->id)
                                ->where('cantidad_peso', '>', 0)
                                ->orderBy('id', 'asc')
                                ->get();
                            ...
                            foreach ($lotes as $lot) {
                                ...
                                $disp = (float) $lot->cantidad_peso;
                                if ($disp <= $porDescontar) {
                                    $porDescontar -= $disp;
                                    $lot->cantidad_peso = 0;
                                    $lot->save();
                                } else {
                                    $lot->cantidad_peso = round($disp - $porDescontar, 3);
                                    $lot->save();
                                    $porDescontar = 0;
                                }
                            }
